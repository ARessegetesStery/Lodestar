...
7. **Testing.** The testing decision written out concretely: coverage and level, framework and test paths, exact commands, the predicted outcome of each criterion with its numbers, the falsifying configuration that turns each criterion red, the break that disables this work's central mechanism and what the whole suite did against it, and the error and edge cases both covered and deliberately not.
